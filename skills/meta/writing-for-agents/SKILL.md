...
description: Meta-skill that governs ALL document writing in the Brain. Use whenever creating or editing any Markdown document — status, handoff, ADR, ficha, guia, relatorio, skill, template — or reviewing one for quality. Triggers: "escrever documento", "criar status/handoff/ficha", "documentar decisão", "atualizar o Brain", "write a doc", "document this".
...
Esta meta-skill define como todo documento do Brain é escrito para que um leitor sem nenhum contexto da sessão — agente ou humano, hoje ou daqui a seis meses — consiga entendê-lo e agir sobre ele sem consultar mais nada. Toda outra skill de escrita do Brain herda estas regras.

## Propósito

Garantir que cada documento do Brain seja autocontido, rastreável e não-ambíguo. O leitor típico não é quem escreveu: é um agente recém-iniciado, sem memória da conversa que gerou o documento. Se o documento só faz sentido para quem estava presente, ele falhou.

## Quando usar

- Antes de criar qualquer arquivo `.md` no Brain, de qualquer tipo.
...
10. **Links relativos, sempre.** Caminho absoluto de máquina em documento do Brain quebra na primeira migração de disco ou de instância. Dentro do Brain, todo link é relativo à posição do arquivo.
...
6. Nenhum caminho absoluto de máquina em links internos do Brain.
...
Casos reais do próprio Brain, generalizados como exemplo negativo:
...
- **Caminhos absolutos que quebram em migração.** Documentos apontando para um repositório no disco antigo depois de o projeto migrar. Cada migração exige caça manual a referências. Correto: links relativos dentro do Brain; caminhos externos citados uma única vez no documento canônico do projeto.
